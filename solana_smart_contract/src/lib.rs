...
pub mod dead_man_switch {
    ...
    pub fn initialize(ctx: Context<Initialize>, deadline: i64, beneficiary: Pubkey, seed: String, will_note: Vec<u8>) -> Result<()> {
    ...
    pub fn claim(ctx: Context<Claim>) -> Result<()> {
        ...
        **ctx.accounts.beneficiary.try_borrow_mut_lamports()? += balance;
        emit!(WillNoteEvent { note: ctx.accounts.escrow.will_note.clone() });
        Ok(())
    }
...
#[derive(Accounts)]
#[instruction(deadline: i64, beneficiary: Pubkey, seed: String, will_note: Vec<u8>)]
pub struct Initialize<'info> {
    #[account(mut)]
    pub owner: Signer<'info>,
    #[account(init, payer = owner, space = 93 + seed.len() + will_note.len(), seeds = [b"escrow", owner.key().as_ref(), seed.as_bytes()], bump)]
...
pub struct Escrow {
    ...
    pub bump: u8,
    pub seed: String,
    pub will_note: Vec<u8>,
}

#[event]
pub struct WillNoteEvent {
    pub note: Vec<u8>,
}
